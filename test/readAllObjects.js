...
function receiveObjectList (err, property) {
  ...
  async.mapSeries(property.value, function (objectId, objectRead) {
    async.mapSeries(['object-name', 'description'], (propertyName, propertyRead) =>
      r.readProperty(process.argv[2], objectId.type, objectId.instance, propertyName, (err, propertyValue) => {
        if (err) {
...
r.on('iam', function (iam) {
  console.log('iam: ', iam)
  console.log('read-property invoke', r.readProperty(process.argv[2], 'device', process.argv[2], 'object-list', receiveObjectList))
})
